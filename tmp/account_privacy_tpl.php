					...
					<div class="col-sm-8">
						<div class="blog-post blog-single-post page-content">
							<div class="single-post-title" style="border-bottom: 2px dotted #CCC; margin-bottom:15px;">
								<div style="float:left;"><h2><?php echo vldext_lang("privacy","app_privacy"); ?></h2></div>
                                
                                <div style="float:right; padding-bottom:5px;">
                                <a href="<?php echo isset($_obj['virtual_path']) ? $_obj['virtual_path'] : "&#123;virtual_path&#125;"; ?>index.php?m=account_privacy" class="btn active"><i class="glyphicon glyphicon-cog"></i> <?php echo vldext_lang("privacy","app_privacy"); ?></a>
						...
						<div class="fieldset">
							<dl class="fieldset">
								<?php if (!empty($_obj['settings'])){ if (!is_array($_obj['settings'])) $_obj['settings']=array(array('settings'=>$_obj['settings'])); $_tmp_arr_keys=array_keys($_obj['settings']); if ($_tmp_arr_keys[0]!='0') $_obj['settings']=array(0=>$_obj['settings']); $_stack[$_stack_cnt++]=$_obj; $_cnt['settings']=count($_obj['settings']); foreach ($_obj['settings'] as $rowcnt=>$settings) { $settings['rowcnt']=$rowcnt; $settings['rowpos']=$rowcnt+1; $settings['rownum']=$rowcnt%2+1; $settings['rowtotal']=$_cnt['settings']; $settings['rowfirst']=$rowcnt==0?1:0; $settings['rowlast']=($rowcnt+1)==$_cnt['settings']?1:0; $_obj=&$settings; ?>
									<?php if ( $this->objval($_obj,'setting_label')  != "access_guestbook_moderate") { ?>
										<dt><label for="field_<?php echo isset($_obj['setting_label']) ? $_obj['setting_label'] : "&#123;setting_label&#125;"; ?>"><?php echo isset($_obj['setting_name']) ? $_obj['setting_name'] : "&#123;setting_name&#125;"; ?></label></dt>
									<?php } ?>
									<?php if ( $this->objval($_obj,'setting_label')  == "access_profile") { ?>
										<dd><select class="select form-control" name="<?php echo isset($_obj['setting_label']) ? $_obj['setting_label'] : "&#123;setting_label&#125;"; ?>" id="field_<?php echo isset($_obj['setting_label']) ? $_obj['setting_label'] : "&#123;setting_label&#125;"; ?>"><?php echo vldext_dropdownlist($_stack[0]['accesstypeprofile'],$_obj['setting_value']); ?></select></dd>
									<?php } elseif ( $this->objval($_obj,'setting_label')  == "access_guestbook_moderate") { ?>
										<dt>
											<label for="field_<?php echo isset($_obj['setting_label']) ? $_obj['setting_label'] : "&#123;setting_label&#125;"; ?>">
												<input type="checkbox" class="checkbox form-control" name="<?php echo isset($_obj['setting_label']) ? $_obj['setting_label'] : "&#123;setting_label&#125;"; ?>" value="1" id="field_<?php echo isset($_obj['setting_label']) ? $_obj['setting_label'] : "&#123;setting_label&#125;"; ?>" <?php if ( $this->objval($_obj,'setting_value') ) { ?>checked="checked"<?php } ?> />
												<?php echo isset($_obj['setting_name']) ? $_obj['setting_name'] : "&#123;setting_name&#125;"; ?>
											</label>
										</dt>
									<?php } else { ?>
										<dd><select class="select form-control" name="<?php echo isset($_obj['setting_label']) ? $_obj['setting_label'] : "&#123;setting_label&#125;"; ?>" id="field_<?php echo isset($_obj['setting_label']) ? $_obj['setting_label'] : "&#123;setting_label&#125;"; ?>"><?php echo vldext_dropdownlist($_stack[0]['accesstypebox'],$_obj['setting_value']); ?></select></dd>
									<?php } ?>
								<?php } $_obj=$_stack[--$_stack_cnt];} ?>
    							<dd class="submit"><input class="submit btn" type="submit" name="submit" value="<?php echo vldext_lang("privacy","submit"); ?>" /></dd>
							</dl>
						</div>
					</div>
				</div>
